...
{"action":"add_task","title":"...","due":"DD/MM/YYYY HH:MM","priority":"high/medium/low"}
...
חשוב: דבר תמיד בעברית, היה ידידותי ותומך, עקוב אחרי משימות פתוחות."""


# ─── Helpers ─────────────────────────────────────────────────────────────────
def get_store(uid: int) -> dict:
    if uid not in user_data:
        user_data[uid] = {"tasks": [], "sessions": [], "history": [],
                          "tc": 0, "sc": 0}
...
# ─── Groq API call ───────────────────────────────────────────────────────────
def call_groq(uid: int, user_msg: str) -> str:
    store = get_store(uid)
    now_str = datetime.now().strftime("%d/%m/%Y %H:%M")

    context = (
        f"[{now_str}] משימות פתוחות: {tasks_text(uid)}\n"
        f"אימונים קרובים: {sessions_text(uid)}\n"
    )

    messages = [{"role": "system", "content": SYSTEM_PROMPT}]
    for turn in store["history"][-16:]:
    ...
    store["history"].append({"role": "user", "content": user_msg})
...
async def schedule_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    now = datetime.now()
    upcoming = sorted(
        [(datetime.strptime(s["datetime"], "%d/%m/%Y %H:%M"), s)
         for s in store["sessions"]
         if _safe_parse(s["datetime"])],
        key=lambda x: x[0]
    )
    upcoming = [(dt, s) for dt, s in upcoming if dt >= now]
    if not upcoming:
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        reply = call_groq(uid, update.message.text)
