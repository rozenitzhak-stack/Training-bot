...
# ─── Grok API call ───────────────────────────────────────────────────────────
def call_grok(uid: int, user_msg: str) -> str:
    store = get_store(uid)
    ...
    # Add history (keep last 16 turns)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        reply = call_grok(uid, update.message.text)
        ...
        await update.message.reply_text(f"❌ שגיאה: {str(e)[:100]}")
